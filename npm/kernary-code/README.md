...
Use `/provider add` for a named custom OpenAI-compatible endpoint (name → URL → secure key → model discovery → default selection), `/provider switch` to switch providers, and `/model` to switch within the current provider. `/vector setup` manages a global embedding-provider catalog: Voyage AI and Jina AI need only a key and use built-in model IDs; named custom providers accept multiple model IDs. Every selected model must return a valid numeric embedding before it can be activated. Use `/vector provider` and `/vector model` for two-level switching. Every project performs a startup health check while keeping memory/vector data project-local. `/language en|zh-CN|zh-TW|ja` switches the customized language pack.
