...
const EN: LanguagePack = LanguagePack {
    ...
    vector_setup_title: "Vector setup · one global embedding provider",
    vector_setup_description: "The provider is global while vector data stays project-local. Dimensions are detected automatically, with manual fallback.",
...
const ZH_CN: LanguagePack = LanguagePack {
    ...
    vector_setup_title: "向量设置 · 全局唯一 Embedding Provider",
    vector_setup_description: "Provider 全局复用，向量数据仍按项目隔离；维度先自动检测，失败后再手动输入。",
    vector_url_prompt: "Embedding API Base URL",
...
const ZH_TW: LanguagePack = LanguagePack {
    ...
    vector_setup_title: "向量設定 · 全域唯一 Embedding Provider",
    vector_setup_description: "Provider 全域共用，向量資料仍按專案隔離；維度先自動偵測，失敗後再手動輸入。",
    vector_url_prompt: "Embedding API Base URL",
...
const JA: LanguagePack = LanguagePack {
    ...
    vector_setup_title: "ベクトル設定 · グローバルで1つの埋め込みプロバイダー",
    vector_setup_description: "Provider は全プロジェクトで共有し、ベクトルデータはプロジェクト別に保持します。次元は自動検出し、失敗時のみ手動入力します。",
